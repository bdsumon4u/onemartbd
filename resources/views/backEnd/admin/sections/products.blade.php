                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        <div class="card">
                            ...
                            <div class="card-body">
                                <form action="{{ route('admin.sections.products.add', $section->id) }}" method="post"
                                    id="add_product_form">
                                    @csrf
                                    ...
                                    <div id="selected_product_info" class="mb-3" style="display: none;">
                                        <div
                                            ...
                                            <button type="button" class="btn btn-sm btn-outline-danger"
                                                id="clear_selection">&times;</button>
                                        </div>
                                    </div>
                    ...
                    <div class="col-12">
                        <div class="card">
                            ...
                            <div class="card-body table-responsive">
                                <form action="{{ route('admin.sections.products.reorder', $section->id) }}" method="post"
                                    id="reorder_products_form">
                                    @csrf
                                    <table class="table table-bordered text-center table-striped">
                                        ...
                                        <tbody id="sortable_products">
                                            @if ($section->products->count() > 0)
                                                @foreach ($section->products as $key => $product)
                                                    <tr data-id="{{ $product->id }}">
                                                        ...
                                                        <td>
                                                            <img width="40"
                                                                src="{{ $product->get_thumb ? asset($product->get_thumb->file_url) : asset('frontEnd/images/no_image.png') }}"
                                                                alt="">
                                                        </td>
                                                        <td>{{ $product->name }}</td>
                                                        <td>{{ $product->sku }}</td>
                                                        ...
                                                        <td>
                                                            <a href="{{ route('admin.sections.products.remove', [$section->id, $product->id]) }}"
                                                                onclick="return confirm('Remove this product from section?')"
                                            ...
                                            @else
                                                <tr>
                                                    <td colspan="8" class="text-center text-danger font-weight-bold">No
                                                        Products Added!</td>
                                                </tr>
                                            @endif
                                        </tbody>
                                    ...
                                    @if ($section->products->count() > 1)
                                        <button type="submit" class="btn btn-primary btn-sm" id="save_product_order_btn"
                                            style="display: none;">
        ...
        $(document).ready(function() {
                        ...
                        success: function(data) {
                            ...
                            if (data.length > 0) {
                                data.forEach(function(product) {
                                    ...
                                    var price = product.sale_price > 0 ? product
                                        .sale_price : product.price;
                                    html +=
                                        '<div class="search-item" data-id="' +
                                        product.id + '" data-name="' + product
                                        .name + '">';
                                    html += '<img src="' + imgUrl + '" alt="">';
                                    html += '<div><strong>' + product.name +
                                        '</strong><br><small>SKU: ' + (product
                                            .sku || 'N/A') + ' | Price: ' +
                                        price + '</small></div>';
                                    html += '</div>';
                                });
            ...
            if (el && el.children.length > 1) {
                Sortable.create(el, {
                    ...
                    animation: 150,
                    onEnd: function() {
                        document.getElementById('save_product_order_btn').style.display =
                        'inline-block';
                    }
                });
            }
