...
@php
    ...
    // $total_completed_orders= $data['total_completed_orders'] ?? [];
    // $total_completed_amount = $data['total_completed_amount'] ?? 0;
        ...
        <div class="dashboard-ecommerce">
            <div class="container-fluid dashboard-content ">
                ...
                <div class="row">
                    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-6 col-12">
                        <div class="page-header">
                            <h2 class="pageheader-title">Trash Orders &nbsp;
                                </a>
                            ...
                            <div class="page-breadcrumb">
                                <nav aria-label="breadcrumb breadcrumbs">
                                    <ol class="breadcrumb">
                                        ...
                                        <li class="breadcrumb-item active" aria-current="page">Trash Orders</li>
                ...
                {{-- <div class="row mb-3 order-card">
                        ...
                        <a
                            ...
                            <div class="card border-3">
                                <div class="card-body">
                                    ...
                                    <h5 class="text-warning h5-s">Total Return</h5>
                                </div>
                            </div>
                        </a>
                    </div>
                        ...
                        <a
                            ...
                            <div class="card border-3">
                                <div class="card-body">
                                    ...
                                    <h5 class="text-success h5-s">Total Pand. Return</h5>
                                </div>
                            </div>
                        </a>
                    </div>
                    <div class="col-xl-2 col-lg-2 col-md-6 col-sm-6 col-6 mb-md-3 mb-2">
                        <a
                            href="{{ Auth::guard('admin')->check() ? route('admin.orders', 'status=Incomplete') : (Auth::guard('manager')->check() ? route('manager.orders', 'status=Incomplete') : (Auth::guard('employee')->check() ? route('employee.orders', 'status=Incomplete') : '')) }}">
                            <div class="card border-3">
                                <div class="card-body">
                                    <div class="metric-value d-inline-block  d-flex align-items-center">
                                        <h2 class="mb-0">{{ $total_incomplete_orders}}</h2>
                                        <h6 class="mb-0 ml-2 total text-primary">৳
                                            {{ $total_incomplete_amount > 0 ? number_format($total_incomplete_amount, 2) : 0 }}
                                        </h6>
                                        <span
                                            class="percentage-badge">{{ $total_order > 0 ? number_format(($total_incomplete_orders/ $total_order) * 100, 2) : 0 }}%</span>
                                    </div>
                                    <h5 class="text-success h5-s">Total Incomplete</h5>
                                </div>
                            </div>
                        </a>
                    </div>
                    <div class="col-xl-2 col-lg-2 col-md-6 col-sm-6 col-6 mb-md-3 mb-2">
                        <a
                            ...
                            <div class="card border-3">
                                <div class="card-body">
                                    <div class="metric-value d-inline-block  d-flex align-items-center">
                                        <h2 class="mb-0">{{ $total_completed_orders}}</h2>
                                        <h6 class="mb-0 ml-2 total text-primary">৳
                                            {{ $total_completed_amount > 0 ? number_format($total_completed_amount, 2) : 0 }}
                                        </h6>
                                        <span
                                            class="percentage-badge">{{ $total_order > 0 ? number_format(($total_completed_orders/ $total_order) * 100, 2) : 0 }}%</span>
                                    </div>
                                    <h5 class="text-success h5-s">Total Completed</h5>
                                </div>
                            </div>
                        </a>
                    </div>
                    ...
                    <div class="row mb-2">
                        <div class="col-md-11 col-12 action_buttons">
                            ...
                            <form action="{{ route('admin.orders.send.to.courier') }}" method="post"
                                id="send_to_courier_form" class="mr-2">
                                @csrf
                            ...
                            <form action="{{ route('admin.orders.all.status') }}" method="post" id="all_status_form"
                                ...
                                <select name="status" id="status" class="form-control h-34">
                                    ...
                                    <option value="11">Pending Return</option>
                                                        ...
                                                        <td>
                                                            ...
                                                            <a target="_blank"
                                                                ...
                                                                <svg xmlns="http://www.w3.org/2000/svg" width="18"
                                                                    ...
                                                                    stroke="#21ae41" stroke-width="2" stroke-linecap="round"
                                                            ...
                                                            @if (Auth::guard('admin')->check())
                                                                <div class="dropdown-menu">
                                                                    ...
                                                                    <a class="dropdown-item {{ $item->status == 7 ? 'd-none' : '' }}"
                                                                        href="{{ Auth::guard('admin')->check() ? route('admin.orders.status', [$item->id, 7]) : (Auth::guard('manager')->check() ? route('manager.orders.status', [$item->id, 7]) : '') }}">Return</a>
                                                                    ...
                                                                    <a class="dropdown-item {{ $item->status == 11 ? 'd-none' : '' }}"
                                                                        href="{{ Auth::guard('admin')->check() ? route('admin.orders.status', [$item->id, 11]) : (Auth::guard('manager')->check() ? route('manager.orders.status', [$item->id, 11]) : '') }}">Pending
                                                                        Retrun</a>
                                                                    <a class="dropdown-item {{ $item->status == 12 ? 'd-none' : '' }}"
                                                                        href="{{ Auth::guard('admin')->check() ? route('admin.orders.status', [$item->id, 12]) : (Auth::guard('manager')->check() ? route('manager.orders.status', [$item->id, 12]) : '') }}">Incomplete</a>
                                                                    <a class="dropdown-item {{ $item->status == 13 ? 'd-none' : '' }}"
                                                                        href="{{ Auth::guard('admin')->check() ? route('admin.orders.status', [$item->id, 13]) : (Auth::guard('manager')->check() ? route('manager.orders.status', [$item->id, 13]) : '') }}">Completed</a>
                                                        ...
                                                        <td class="text-center">
                                                            @if (Auth::guard('admin')->check())
                                                                <a href="{{ route('admin.orders.restore', $item->id) }}"
                                                                    title="Restore" class="d-block mb-1"
                                                                    onclick="return confirm('Are you sure to restore this?')"><i
                                                                        class="fa fa-undo"></i></a>
                                                                <a href="{{ route('admin.orders.force.delete', $item->id) }}"
                                                                    title="Delete" class="d-block mb-1"
                                                            ...
                                                            @elseif($item->status != 6 && $item->status != 1 && $item->status != 7)
                                                                <div class="dropdown-menu">
                                                                    <a class="dropdown-item {{ $item->status == 0 ? 'd-none' : '' }}"
                                                                        ...
                                                                        Hold</a>
                                                                    {{-- <a class="dropdown-item {{ $item->status == 1 ? 'd-none' : '' }}"
                                                                        href="{{ Auth::guard('admin')->check() ? route('admin.orders.status', [$item->id, 1]) : (Auth::guard('employee')->check() ? route('employee.orders.status', [$item->id, 1]) : '') }}">Delivered</a> --}}
                                                                    <a class="dropdown-item {{ $item->status == 2 ? 'd-none' : '' }}"
                                                                    ...
                                                                    <a class="dropdown-item {{ $item->status == 5 ? 'd-none' : '' }}"
                                                                        ...
                                                                        Invoice</a>
                                                                    {{-- <a class="dropdown-item {{ $item->status == 6 ? 'd-none' : '' }}"
                                                                        href="{{ Auth::guard('admin')->check() ? route('admin.orders.status', [$item->id, 6]) : (Auth::guard('employee')->check() ? route('employee.orders.status', [$item->id, 6]) : '') }}">On
                                                                        Delivery</a> --}}
                                                                    {{-- <a class="dropdown-item {{ $item->status == 7 ? 'd-none' : '' }}"
                                                                        href="{{ Auth::guard('admin')->check() ? route('admin.orders.status', [$item->id, 7]) : (Auth::guard('employee')->check() ? route('employee.orders.status', [$item->id, 7]) : '') }}">Return</a> --}}
                                                                    <a class="dropdown-item {{ $item->status == 8 ? 'd-none' : '' }}"
                                                                    ...
                                                                    <a class="dropdown-item {{ $item->status == 11 ? 'd-none' : '' }}"
                                                                        href="{{ route('employee.orders.status', [$item->id, 11]) }}">Pending
                                                                        Retrun</a>
                                                                    <a class="dropdown-item {{ $item->status == 12 ? 'd-none' : '' }}"
                                                                        href="{{ route('employee.orders.status', [$item->id, 12]) }}">Incomplete</a>
                                                                    <a class="dropdown-item {{ $item->status == 13 ? 'd-none' : '' }}"
                                                                        href="{{ route('employee.orders.status', [$item->id, 13]) }}">Completed</a>
                                                                    <a class="dropdown-item {{ $item->status == 14 ? 'd-none' : '' }}"
