	...
	<div class="grid">
		<div class="box">
			...
			<table class="no-border">
				<tr><td style="width:95px"><strong>Name:</strong></td><td>{{ $user?->name }}</td></tr>
				<tr><td><strong>Role:</strong></td><td>{{ $roleLabel }}</td></tr>
				<tr><td><strong>Monthly Salary:</strong></td><td>{{ $formatMoney($user?->monthly_salary ?? 0) }}</td></tr>
				<tr><td><strong>Schedule:</strong></td><td>{{ $user?->start_time ?? '00:00:00' }} - {{ $user?->end_time ?? '23:59:59' }}</td></tr>
				<tr><td><strong>Off Days:</strong></td><td>{{ $offDays ?: 'None' }}</td></tr>
			</table>
		</div>
		<div class="box">
			<div class="box-title">Attendance Summary</div>
			<table class="no-border">
				<tr><td style="width:95px"><strong>Total Days:</strong></td><td>{{ $payroll->total_days }} days</td></tr>
				<tr><td><strong>Present Days:</strong></td><td>{{ $payroll->present_days }} days</td></tr>
				<tr><td><strong>Off-day Work:</strong></td><td>{{ $payroll->off_day_presents }} days</td></tr>
			</table>
		</div>
	</div>
		...
		<tbody>
			@foreach ($attendances as $attendance)
				...
				<tr>
					...
					<td class="red">{{ (float) ($attendance->penalty_amount ?? 0) > 0 ? $formatMoney($attendance->penalty_amount) : '-' }}</td>
	...
	<div class="summary-wrap">
		...
		<div>
			...
			<table class="calc-table" style="width:100%; font-size:12px;">
				<tr>
					<td>Base Salary ({{ $payroll->present_days }} days × {{ $formatMoney($payroll->daily_salary) }})</td>
					<td class="text-right">{{ $formatMoney($payroll->base_salary) }}</td>
				</tr>
				<tr><td class="green">+ Off-day Bonus</td><td class="text-right green">{{ $formatMoney($payroll->off_day_bonus) }}</td></tr>
				<tr><td class="green">+ Overtime Bonus</td><td class="text-right green">{{ $formatMoney($payroll->overtime_amount) }}</td></tr>
				<tr><td class="green">+ Hazira Bonus</td><td class="text-right green">{{ $formatMoney($payroll->hazira_bonus_amount) }}</td></tr>
				<tr><td class="green">+ Special Bonus</td><td class="text-right green">{{ $formatMoney($payroll->occasional_bonus_amount) }}</td></tr>
				<tr><td class="green">+ xSell Bonus</td><td class="text-right green">{{ $formatMoney($payroll->xsell_bonus_amount) }}</td></tr>
				<tr><td class="red">− Late Fee Deduction</td><td class="text-right red">{{ $formatMoney($payroll->late_deduction) }}</td></tr>
				<tr><td class="red">− Penalty Deduction</td><td class="text-right red">{{ $formatMoney($payroll->penalty_amount) }}</td></tr>
				<tr><td class="red">− Advance Deduction</td><td class="text-right red">{{ $formatMoney($payroll->advance_deduction) }}</td></tr>
				<tr>
					<td style="font-weight:700; border-top:2px solid #222;">NET SALARY</td>
					<td class="text-right" style="font-weight:700; border-top:2px solid #222;">{{ $formatMoney($payroll->net_salary) }}</td>
				</tr>
			</table>
		</div>
	</div>
