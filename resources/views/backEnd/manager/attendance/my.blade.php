...
@endsection
@section('js')<script>(function(){const s=document.getElementById('attendanceStatus');function r(){fetch("{{ route('manager.my_attendance.status') }}").then(x=>x.json()).then(d=>s.innerHTML=`Checked In: ${d.is_checked_in?'Yes':'No'} | Checked Out: ${d.is_checked_out?'Yes':'No'} | In: ${d.check_in??'-'} | Out: ${d.check_out??'-'}`)}document.getElementById('attendanceToggle').addEventListener('click',()=>{fetch("{{ route('manager.my_attendance.toggle') }}",{method:'POST',headers:{'X-CSRF-TOKEN':'{{ csrf_token() }}'}}).then(()=>r())});r();})();</script>@endsection
