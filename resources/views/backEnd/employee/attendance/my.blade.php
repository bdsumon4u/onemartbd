                ...
                <tbody>
                    @foreach ($attendances as $a)
                        <tr>
                            ...
                            <td>{{ $a->overtime_minutes + $a->extra_overtime_minutes }}</td>
...
<script>
    (function() {
        ...
        function r() {
            fetch("{{ route('employee.my_attendance.status') }}").then(x => x.json()).then(d => s.innerHTML =
                `Checked In: ${d.is_checked_in?'Yes':'No'} | Checked Out: ${d.is_checked_out?'Yes':'No'} | In: ${d.check_in??'-'} | Out: ${d.check_out??'-'}`
                )
        }
        document.getElementById('attendanceToggle').addEventListener('click', () => {
            fetch("{{ route('employee.my_attendance.toggle') }}", {
                method: 'POST',
                headers: {
