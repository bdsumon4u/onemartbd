...
class AdminController extends Controller
{
    public function dashboard(Request $request)
    {
        ...
        if ($request->input('role_id') == 1 || $request->input('role_id') == 2 || $request->input('role_id') == 3) {
            ...
            $data['total_hold_orders'] = DB::table('orders')->whereBetween('created_at', [\Illuminate\Support\Facades\Date::now()->subDays(7), \Illuminate\Support\Facades\Date::now()])->where('status', 0)->count();
            $data['total_deliver_orders'] = DB::table('orders')->whereBetween('created_at', [\Illuminate\Support\Facades\Date::now()->subDays(2), \Illuminate\Support\Facades\Date::now()])->where('status', 1)->count();
            $data['total_process_orders'] = DB::table('orders')->whereBetween('created_at', [\Illuminate\Support\Facades\Date::now()->subDays(7), \Illuminate\Support\Facades\Date::now()])->where('status', 2)->count();
            $data['total_pend_pay_orders'] = DB::table('orders')->whereBetween('created_at', [\Illuminate\Support\Facades\Date::now()->subDays(7), \Illuminate\Support\Facades\Date::now()])->where('status', 3)->count();
            $data['total_cancel_orders'] = DB::table('orders')->whereBetween('created_at', [\Illuminate\Support\Facades\Date::now()->subDays(2), \Illuminate\Support\Facades\Date::now()])->where('status', 4)->count();
            $data['total_pending_delivery_orders'] = DB::table('orders')->whereBetween('created_at', [\Illuminate\Support\Facades\Date::now()->subDays(3), \Illuminate\Support\Facades\Date::now()])->where('status', 5)->count();
            $data['total_on_delivery_orders'] = DB::table('orders')->whereBetween('created_at', [\Illuminate\Support\Facades\Date::now()->subDays(3), \Illuminate\Support\Facades\Date::now()])->where('status', 6)->count();
            $data['total_return_orders'] = DB::table('orders')->whereBetween('created_at', [\Illuminate\Support\Facades\Date::now()->subDays(7), \Illuminate\Support\Facades\Date::now()])->where('status', 7)->count();
            $data['total_courier_hold_orders'] = DB::table('orders')->whereBetween('created_at', [\Illuminate\Support\Facades\Date::now()->subDays(7), \Illuminate\Support\Facades\Date::now()])->where('status', 8)->count();
            $data['total_nr_1_orders'] = DB::table('orders')->whereBetween('created_at', [\Illuminate\Support\Facades\Date::now()->subDays(7), \Illuminate\Support\Facades\Date::now()])->where('status', 9)->count();
            $data['total_nr_2_orders'] = DB::table('orders')->whereBetween('created_at', [\Illuminate\Support\Facades\Date::now()->subDays(7), \Illuminate\Support\Facades\Date::now()])->where('status', 10)->count();
            ...
            $data['total_orders'] = DB::table('order_assigns')
                ...
                ->count();
            $data['total_hold_orders'] = DB::table('order_assigns')
                ...
                ->count();
            $data['total_deliver_orders'] = DB::table('order_assigns')
                ...
                ->count();
            $data['total_process_orders'] = DB::table('order_assigns')
                ->leftJoin('orders', 'orders.id', 'order_assigns.order_id')->whereBetween('orders.created_at', [\Illuminate\Support\Facades\Date::now()->subDays(7), \Illuminate\Support\Facades\Date::now()])
                ->where([['order_assigns.employee_id', $emp_id], ['status', 2]])
                ->count();
            $data['total_pend_pay_orders'] = DB::table('order_assigns')
                ->leftJoin('orders', 'orders.id', 'order_assigns.order_id')->whereBetween('orders.created_at', [\Illuminate\Support\Facades\Date::now()->subDays(7), \Illuminate\Support\Facades\Date::now()])
                ->where([['order_assigns.employee_id', $emp_id], ['status', 3]])
                ->count();
            $data['total_cancel_orders'] = DB::table('order_assigns')
                ->leftJoin('orders', 'orders.id', 'order_assigns.order_id')->whereBetween('orders.created_at', [\Illuminate\Support\Facades\Date::now()->subDays(2), \Illuminate\Support\Facades\Date::now()])
                ->where([['order_assigns.employee_id', $emp_id], ['status', 4]])
                ->count();
            $data['total_pending_delivery_orders'] = DB::table('order_assigns')
                ->leftJoin('orders', 'orders.id', 'order_assigns.order_id')->whereBetween('orders.created_at', [\Illuminate\Support\Facades\Date::now()->subDays(3), \Illuminate\Support\Facades\Date::now()])
                ->where([['order_assigns.employee_id', $emp_id], ['status', 5]])
                ->count();
            $data['total_on_delivery_orders'] = DB::table('order_assigns')
                ->leftJoin('orders', 'orders.id', 'order_assigns.order_id')->whereBetween('orders.created_at', [\Illuminate\Support\Facades\Date::now()->subDays(3), \Illuminate\Support\Facades\Date::now()])
                ->where([['order_assigns.employee_id', $emp_id], ['status', 6]])
                ->count();
            $data['total_return_orders'] = DB::table('order_assigns')
                ->leftJoin('orders', 'orders.id', 'order_assigns.order_id')->whereBetween('orders.created_at', [\Illuminate\Support\Facades\Date::now()->subDays(7), \Illuminate\Support\Facades\Date::now()])
                ->where([['order_assigns.employee_id', $emp_id], ['status', 7]])
                ->count();
            $data['total_courier_hold_orders'] = DB::table('order_assigns')
                ->leftJoin('orders', 'orders.id', 'order_assigns.order_id')->whereBetween('orders.created_at', [\Illuminate\Support\Facades\Date::now()->subDays(7), \Illuminate\Support\Facades\Date::now()])
                ->where([['order_assigns.employee_id', $emp_id], ['status', 8]])
                ->count();
            $data['total_nr_1_orders'] = DB::table('order_assigns')
                ->leftJoin('orders', 'orders.id', 'order_assigns.order_id')->whereBetween('orders.created_at', [\Illuminate\Support\Facades\Date::now()->subDays(7), \Illuminate\Support\Facades\Date::now()])
                ->where([['order_assigns.employee_id', $emp_id], ['status', 9]])
                ->count();
            $data['total_nr_2_orders'] = DB::table('order_assigns')
                ->leftJoin('orders', 'orders.id', 'order_assigns.order_id')->whereBetween('orders.created_at', [\Illuminate\Support\Facades\Date::now()->subDays(7), \Illuminate\Support\Facades\Date::now()])
