                            ...
                            <div class="col-md-4 ">
                                <form action="" method="get" class="d-flex">
                                    ...
                                    <a href="{{ route('admin.orders.return.receive.clear') }}"
                                       style="margin-left: 10px; color: white;background-color:#000; padding: 5px 10px;  ">Clear</a>
                                </form>

                                <h4 class="my-2 text-danger">Total Received: {{session()->has('return_received_orders')?count(session('return_received_orders')):0}}</h4>

                                <ul class="list-unstyled mt-2">
                                    @if (session()->has('return_received_orders'))
                                        @foreach (session('return_received_orders') as $key => $item)
                                            <li style="margin-bottom: 3px;"><b>{{ ++$key }}</b> &emsp;{{ $item }}</li>
                                        @endforeach
                                    @endif
                                </ul>
                            </div>
